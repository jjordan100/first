# this is new text by Jim Jordan 11/15/2017 10:35PM EST
...
resourcegroup_name = 'jrj'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'jrj'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
carsinv.color = 'Black'
carsinv.price = 24215
print('Created entry for Lexus GX470...\n')
time.sleep(1)
...
coffeeinv = Entity()
coffeeinv.PartitionKey = 'coffee'
coffeeinv.RowKey = '001'
coffeeinv.brand = 'Starbucks'
coffeeinv.flavor = 'Verona'
coffeeinv.size = '8oz'
coffeeinv.price = 1.50
print('Created entry for Starbucks Verona...\n')
time.sleep(1)

coffeeinv = Entity()
coffeeinv.PartitionKey = 'coffee'
coffeeinv.RowKey = '002'
coffeeinv.brand = 'Starbucks'
coffeeinv.flavor = 'Italian'
coffeeinv.size = '8oz'
coffeeinv.price = 1.50
print('Created entry for Starbucks Italian...\n')
time.sleep(1)

coffeeinv = Entity()
coffeeinv.PartitionKey = 'coffee'
coffeeinv.RowKey = '002'
coffeeinv.brand = 'Folgers'
coffeeinv.flavor = 'Bold'
coffeeinv.size = '8oz'
coffeeinv.price = 1.00
print('Created entry for Folgers Bold...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'cars'", select='make,model,cost')
for item in items:
    print('Make: ' + item.make)
    print('Model: ' + item.model)
    print('Cost: ' + str(item.price) + '\n')

items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffee'", select='brand,flavor, price')
for item in items:
